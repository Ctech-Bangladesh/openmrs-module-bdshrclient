...
public class EncounterPush implements EventWorker {
  ...
  private final CompositionBundleCreator compositionBundleCreator;
  private final EncounterService encounterService;
  private final PropertiesReader propertiesReader;
  private final ClientRegistry clientRegistry;
  private final SHRClient shrClient;
  private final List<String> encounterUuidsProcessed;
  ...
  public EncounterPush(EncounterService encounterService, PropertiesReader propertiesReader,
      ...
      ClientRegistry clientRegistry, SystemUserService systemUserService)
  ...
  public void process(Event event) {
    ...
    try {
      ...
      SystemProperties systemProperties = new SystemProperties(propertiesReader.getFrProperties(),
          propertiesReader.getTrProperties(), propertiesReader.getPrProperties(),
          propertiesReader.getFacilityInstanceProperties(), propertiesReader.getMciProperties(),
          propertiesReader.getShrProperties(), propertiesReader.getFhirMappingProperties());
  ...
  private void saveIdMappingsForOrders(Set<Order> orders, String healthId, String shrEncounterId,
    ...
    for (Order order : orders) {
      if (order.getOrderType().getUuid().equals(OrderType.DRUG_ORDER_TYPE_UUID)) {
        orderUrlReferenceIds.put(EntityReference.REFERENCE_RESOURCE_NAME,
            new MedicationRequest().getResourceType().name());
        saveOrderIdMapping(shrEncounterId, order.getUuid(), IdMappingType.MEDICATION_ORDER,
            orderUrlReferenceIds, systemProperties);
      } else if (order.getOrderType().getName().equals(MRSProperties.MRS_PROCEDURE_ORDER_TYPE)) {
        orderUrlReferenceIds.put(EntityReference.REFERENCE_RESOURCE_NAME,
            new ProcedureRequest().getResourceType().name());
        saveOrderIdMapping(shrEncounterId, order.getUuid(), IdMappingType.PROCEDURE_REQUEST,
            orderUrlReferenceIds, systemProperties);
  ...
  private void saveOrderIdMapping(String shrEncounterId, String orderUuid, String idMappingType,
      HashMap<String, String> orderUrlReferenceIds, SystemProperties systemProperties) {
    orderUrlReferenceIds.put(EntityReference.REFERENCE_ID, orderUuid);
    String orderUrl = new EntityReference().build(BaseResource.class, systemProperties,
        orderUrlReferenceIds);
    String externalId = String.format(MRSProperties.RESOURCE_MAPPING_EXTERNAL_ID_FORMAT,
        shrEncounterId, orderUuid);
    idMappingsRepository.saveOrUpdateIdMapping(
        new OrderIdMapping(orderUuid, externalId, idMappingType, orderUrl, new Date()));
  }

  private void saveIdMappingForDiagnosis(Encounter openMrsEncounter, String healthId,
      String shrEncounterId, SystemProperties systemProperties) {
    ...
    conditionUrlReferenceIds.put(EntityReference.REFERENCE_RESOURCE_NAME,
        new Condition().getResourceType().name());
    ...
    for (Obs obs : openMrsEncounter.getObsAtTopLevel(false)) {
      ...
      if (!compoundObservation.isOfType(ObservationType.VISIT_DIAGNOSES)) {
        continue;
      }
      ...
      String diagnosisUrl = entityReference.build(BaseResource.class, systemProperties,
          conditionUrlReferenceIds);
      String externalId = String.format(MRSProperties.RESOURCE_MAPPING_EXTERNAL_ID_FORMAT,
          shrEncounterId, obs.getUuid());
      idMappingsRepository.saveOrUpdateIdMapping(
          new DiagnosisIdMapping(obs.getUuid(), externalId, diagnosisUrl));
    }
  }

  private void saveEncounterIdMapping(String openMrsEncounterUuid, String healthId,
      String shrEncounterId, SystemProperties systemProperties) {
    String shrEncounterUrl = getShrEncounterUrl(healthId, shrEncounterId, systemProperties);
    idMappingsRepository.saveOrUpdateIdMapping(
        new EncounterIdMapping(openMrsEncounterUuid, shrEncounterId, shrEncounterUrl, new Date()));
  }

  private String getShrEncounterUrl(String healthId, String shrEncounterId,
      SystemProperties systemProperties) {
  ...
  private boolean shouldUploadEncounter(Encounter openMrsEncounter) {
    ...
    if (encounterUuidsProcessed.contains(openMrsEncounter.getUuid())) {
      log.debug(String.format("Enounter[%s] has been processed in the same job run before.",
          openMrsEncounter.getUuid()));
      return false;
    }
    return true;
  }

  private String pushEncounterCreate(Encounter openMrsEncounter, String healthId,
      SystemProperties systemProperties) throws IOException {
    try {
      String encPathPattern = StringUtil.removePrefix(
          propertiesReader.getShrPatientEncPathPattern(), "/");
  ...
  private void pushEncounterUpdate(Encounter openMrsEncounter, String shrEncounterId,
      String healthId, SystemProperties systemProperties) throws IOException {
    try {
      String encPathPattern = StringUtil.removePrefix(
          propertiesReader.getShrPatientEncPathPattern(), "/");
  ...
  private String getEncounterIdFromResponse(String shrEncounterResponse)
  ...
  private String getPatientHealthId(Patient emrPatient) {
    PatientIdMapping patientIdMapping = (PatientIdMapping) idMappingsRepository.findByInternalId(
        emrPatient.getUuid(), IdMappingType.PATIENT);
    if (patientIdMapping == null) {
      throw new AtomFeedClientException(
          String.format("Patient [%s] is not yet synced to MCI.", emrPatient.getUuid()));
    }
