...
public class FacilityPull {
  ...
  public FacilityPull(PropertiesReader propertiesReader, RestClient frWebClient,
      LocationService locationService, ScheduledTaskHistory scheduledTaskHistory,
      IdMappingRepository idMappingRepository, LocationMapper locationMapper,
    ...
    this.shrLocationTag = locationService.getLocationTag(
        omrsLocationService.getHIEFacilityLocationTag());
...
//    logger.info(frLocationEntries.size() + " entries updated");
  ...
  private List<FRLocationEntry> synchronizeUpdates() throws IOException {
    ...
    String feedUriForLastReadEntry = scheduledTaskHistory.getFeedUriForLastReadEntryByFeedUri(
        FR_FACILITY_LEVEL_FEED_URI);
    int offset;
    ...
    do {
      ...
      if (lastRetrievedPartOfList != null) {
        ...
        && noOfEntriesSynchronizedSoFar < MAX_NUMBER_OF_ENTRIES_TO_BE_SYNCHRONIZED);
    ...
    if (lastRetrievedPartOfList.size() == DEFAULT_LIMIT) {
      ...
      scheduledTaskHistory.setFeedUriForLastReadEntryByFeedUri(
          facilityResourceRefPath + StringUtil.removePrefix(nextCompleteContextPath, "/"),
          FR_FACILITY_LEVEL_FEED_URI);
    } else {
    ...
    if (!synchronizedLocationEntries.isEmpty()) {
      FRLocationEntry frLocationEntry = lastRetrievedPartOfList.get(
          lastRetrievedPartOfList.size() - 1);
      scheduledTaskHistory.setLastReadEntryId(frLocationEntry.getId(),
          FR_FACILITY_LEVEL_FEED_URI);
    }

    logger.info(synchronizedLocationEntries.size() + " entries synchronized");
    logger.info(
        failedDuringSaveOrUpdateOperation.size() + " entries failed during synchronization");
  ...
  private void createNewLocation(FRLocationEntry frLocationEntry) {
    ...
    try {
      ...
      facilityCatchmentRepository.saveMappings(location.getLocationId(),
  ...
  private void saveOrUpdateFacilityEntries(List<FRLocationEntry> frLocationEntries) {
    for (FRLocationEntry frLocationEntry : frLocationEntries) {
      IdMapping facilityIdMapping = idMappingsRepository.findByExternalId(frLocationEntry.getCode(),
      ...
      if (facilityIdMapping == null) {
        Location existingLocation = locationService.getLocation(locationName);
        if (existingLocation != null) {
          idMappingsRepository.saveOrUpdateIdMapping(
              new IdMapping(existingLocation.getUuid(), frLocationEntry.getCode(),
                  IdMappingType.FACILITY, locationUrl, new Date()));
          facilityIdMapping = idMappingsRepository.findByExternalId(frLocationEntry.getCode(),
              IdMappingType.FACILITY);
          if (facilityIdMapping != null) {
